...
\begin{tikzpicture}[
  ...
  compose/.style={
    {Diamond[fill=black, length=3mm, width=2mm]}-,
    thick
  },
  depend/.style={
    dashed,
  ...
  % Nodes
  \node[class] (Network) {\textbf{Network} \\ \textit{Manages Layers}};
  \node[class, below=1.5cm of Network] (Layer) {\textbf{Layer} \\ \textit{Abstract Base}};
  \node[class, left=2.5cm of Layer] (Tensor) {\textbf{Tensor} \\ \textit{4D Array}};
  
  % Derived layers
  \node[class, below left=1.5cm and 1cm of Layer] (Conv2d) {\textbf{Conv2d}};
  \node[class, below=1.5cm of Layer] (Dense) {\textbf{Dense}};
  \node[class, below right=1.5cm and 1cm of Layer] (ReLU) {\textbf{ReLU}};
  \node[class, right=0.5cm of ReLU] (Others) {\textbf{...}};

  % Relationships
  \draw[compose] (Network) -- node[left, align=right] {owns} (Layer);
  \draw[depend] (Layer) -- node[above] {uses} (Tensor);
  
  % Inheritance
  \draw[inherit] (Conv2d) |- ([yshift=-0.7cm]Layer.south) -- (Layer.south);
  \draw[inherit] (Dense) -- (Layer);
  \draw[inherit] (ReLU) |- ([yshift=-0.7cm]Layer.south) -- (Layer.south);
  \draw[inherit] (Others) |- ([yshift=-0.7cm]Layer.south) -- (Layer.south);
...
The training process involves iterating over the dataset for a specified number of epochs. For each mini-batch:
\begin{enumerate}
