...
{
  if (A.totalColumns != B.totalRows || A.totalBatchSize != B.totalBatchSize || A.totalChannels != B.totalChannels)
  {
...
{
  std::cout << name << " (" << totalBatchSize << "x" << totalChannels << "x" << totalRows << "x" << totalColumns << "):" << std::endl;
  for (size_t n = 0; n < std::min(totalBatchSize, (size_t)1); ++n)
  {
    for (size_t c = 0; c < std::min(totalChannels, (size_t)1); ++c)
    {
      for (size_t i = 0; i < std::min(totalRows, (size_t)5); ++i)
      {
        ...
        if (totalColumns > 5) std::cout << "...";
        std::cout << std::endl;
      }
      if (totalRows > 5) std::cout << "..." << std::endl;
    }
  }
  std::cout << std::endl;
}
